...
exports.run = async (client, message, args) => {
  ...
  let gunluk = await db.fetch(
    `gunluk_${message.guild.id}_${message.author.id}`
  );

  if (gunluk !== null && timeout - (Date.now() - gunluk) > 0) {
    let time = ms(timeout - (Date.now() - gunluk));
    return message.channel.send(
      `:warning: Zaten günlük ödülünü almışsın. ${time.days} gün, ${time.hours} saat, ${time.minutes} dakika, ve ${time.seconds} saniye içinde geri dön. :warning: `
    );
  } else {
    await db.add(`bakiye_${silinecekkllnc.id}`, eklenecek);
